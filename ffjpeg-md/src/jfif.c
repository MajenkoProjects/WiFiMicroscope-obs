...
/* 内部函数实现 */
#if DEBUG_JFIF
void jfif_dump(void *jfifv)
{
	JFIF *jfif = (JFIF *)jfifv;
    int i, j;
...
void* jfif_load(uint8_t *data, size_t len)
{
    ...
    fp = md_fopen(data, len);
    if (!fp) goto done;

    while (1) {
        do { header = md_fgetc(fp); } while (header != EOF && header != 0xff); // get header
        do { type   = md_fgetc(fp); } while (type   != EOF && type   == 0xff); // get type
        ...
        size = (int)md_fread(buf, 1, size, fp);
        switch (type) {
        case 0xc0: // SOF0
...
read_data:
    md_fseek(fp, 0, SEEK_END);
    jfif->datalen = md_ftell(fp) - offset;
    jfif->databuf = malloc(jfif->datalen);
    if (jfif->databuf) {
        md_fseek(fp, offset, SEEK_SET);
        md_fread(jfif->databuf, 1, jfif->datalen, fp);
    }
